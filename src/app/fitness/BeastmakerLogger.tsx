...
export function BeastmakerLogger() {
  ...
  // Timer tick — counts from -2 upward
  useEffect(() => {
    if (phase === "countdown" || phase === "running") {
      intervalRef.current = setInterval(() => {
        setElapsed((s) => {
          const next = s + 1;
          // When the countdown hits 0, record the actual start time
          if (next === 0) setStartTime(new Date());
          // Transition phase label at 0 for visual purposes
    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          ...
          <p className="text-xs text-muted-foreground">Logg klatreøkt · fingerboard timer</p>
        </div>
      </div>
